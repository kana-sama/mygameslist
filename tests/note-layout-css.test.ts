...
describe("note column layout", () => {
  ...
  it("defines the exact desktop rich-tooltip card, arrow, header, body, and definition rows", () => {
    ...
    expect(productionStyles).toMatch(/\.markdown-rich-tooltip--palette\s*\{[^}]*z-index:\s*130;/s);
    expect(productionStyles).toMatch(/\.markdown-rich-tooltip--palette\.markdown-rich-tooltip--desktop\s*\{[^}]*position:\s*fixed;/s);
    expect(productionStyles).toMatch(/\.markdown-rich-tooltip--desktop\s*\{[^}]*position:\s*absolute;[^}]*width:\s*344px;[^}]*max-height:\s*var\(--markdown-rich-tooltip-max-height\);[^}]*overflow:\s*visible;/s);
