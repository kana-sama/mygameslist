...
Позиция пересчитывается при прокрутке заметки, прокрутке страницы, resize viewport, изменении размеров note surface и изменении размеров tooltip content. Прокрутка не закрывает карточку. Если source временно выходит за видимую область scrollable note, карточка остаётся открытой и прижимается к соответствующей вертикальной границе; стрелка также остаётся в допустимых пределах.
...
- квадрат `11px × 11px`, повёрнутый на `45deg`;
- использует тот же background и border, что карточка;
