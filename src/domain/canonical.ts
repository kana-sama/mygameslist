...
function serialize(value: unknown, seen: Set<object>): string {
  ...
  try {
    ...
    return `{${keys.map((key) => `${JSON.stringify(key)}:${serialize(object[key], seen)}`).join(",")}}`;
  } finally {
    seen.delete(value);
  }
}

/** Stable compact JSON: object keys are recursively sorted, array order is retained. */
...
/** Small dependency-free SHA-256 implementation, usable synchronously in Safari and Node. */
...
export function computeLibraryRevision(database: LibraryDatabase): string {
  return canonicalHash({ ...database, revision: "" });
}

export function withComputedRevision(database: LibraryDatabase): LibraryDatabase {
  const copy = structuredClone(database);
  copy.revision = computeLibraryRevision(copy);
  return copy;
}
