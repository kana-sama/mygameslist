...
import { DEFAULT_NOTE_GROUP_RANK, STATUS_IDS, TIER_IDS, type Asset, type Game, type InteractiveNoteFieldUpdate, type Note, type NoteAttachment, type StatusId, type TierId } from "../domain";
...
function canAppendNoteGroupRank(groupRank: number): boolean {
  const nextRank = groupRank + NOTE_GROUP_RANK_STEP;
  return Number.isSafeInteger(nextRank) && nextRank >= 0;
}

function appendNoteGroupRank(groupRank: number): number {
  if (!canAppendNoteGroupRank(groupRank)) throw new RangeError("Невозможно выделить ранг группы заметок");
  return groupRank + NOTE_GROUP_RANK_STEP;
}
...
function canPrepareNoteGroupAfter(notes: EditableNote[], leftGroupRank: number): boolean {
  try {
    prepareNoteGroupAfter(notes, leftGroupRank);
    return true;
  } catch (reason) {
...
function editableNotesForGame(game: Game | undefined, notes: Note[]): EditableNote[] {
  let editable = [...notes]
    ...
    .map((note) => ({
      ...
      ...(note.doubleHeight ? { doubleHeight: true } : {}),
      ...(note.doubleWidth ? { doubleWidth: true } : {}),
...
const GameLinkSuggestionsContext = createContext<GameLinkCompletionContextValue>({ games: [] });
...
function PlainNoteEditor({
  ...
  onProcessingChange?: (processing: boolean, draft: EditableNote) => void;
  onSubmit?: (draft: EditableNote) => void;
}) {
  ...
  }, [autoFocus, note.clientId, onAutoFocusConsumed]);
  useEffect(() => {
    active.current = true;
    return () => { active.current = false; if (pendingJobs.current > 0) processingChange.current?.(false, noteRef.current); };
  }, []);
  useEffect(() => {
    if (attachmentPickerOpen) attachmentFirstAction.current?.focus();
  }, [attachmentPickerOpen]);

  const publishNote = (next: EditableNote) => {
  ...
  const enqueueAttachmentFiles = (files: File[], mode: "image" | "file" | "auto") => {
    ...
    if (wasIdle) { setProcessingImages(true); processingChange.current?.(true, noteRef.current); }
    ...
    void task.finally(() => {
      pendingJobs.current -= 1;
      if (pendingJobs.current === 0 && active.current) { setProcessingImages(false); processingChange.current?.(false, noteRef.current); }
    }).catch(() => undefined);
  };
  ...
  const addYouTubeAttachment = () => {
    ...
    const current = noteRef.current;
    publishNote({ ...current, attachments: [...current.attachments, { type: "link", url: canonicalUrl, label: "YouTube" }] });
    closeYouTubeInput();
  };

  const toggleSize = (field: "doubleHeight" | "doubleWidth") => {
...
function InlineNoteCard({ note, index, count, editing, editorAutoFocus, actionsDisabled, sortingDisabled, dropIndicatorEdge, assets, storageLocked, saving, canAddBlob, resolveAssetUrl, takeInitialFiles, onEdit, onChange, onSave, onTaskSave, onCancel, onDelete, onMove, taskError, completedChecklistFilterEnabled, completedChecklistFilterGeneration = 0, completedChecklistFilterRevision = 0 }: InlineNoteCardProps) {
  ...
  const completedChecklistFilterSnapshot = completedChecklistFilterEnabled
    && completedChecklistFilterSnapshotCache.current?.epoch === completedChecklistFilterEpoch
    ? completedChecklistFilterSnapshotCache.current.snapshot
  ...
  const saveCheckboxBodyMarkdown = useCallback((bodyMarkdown: string) => {
    void onTaskSave({ ...note, bodyMarkdown }, "checkbox");
  }, [note, onTaskSave]);
  ...
  return <SortableNoteCard actionsDisabled={actionsDisabled} assets={assets} completedChecklistFilterEnabled={completedChecklistFilterEnabled} completedChecklistFilterRevision={completedChecklistFilterRevision} completedChecklistFilterSnapshot={completedChecklistFilterSnapshot} disabled={sortingDisabled} dropIndicatorEdge={dropIndicatorEdge} note={note} onCollapsedChecklistSectionsChange={saveCollapsedChecklistSections} onEdit={onEdit} onTaskChange={saveBodyMarkdown} onTaskCheckboxChange={saveCheckboxBodyMarkdown} resolveAssetUrl={resolveAssetUrl} taskChangesDisabled={saving} taskError={taskError} />;
}
...
function ConnectedInlineNoteCard({ noteInteractionSource, ...props }: InlineNoteCardProps & { noteInteractionSource: NoteInteractionSource; note: EditableNote & { id: string } }) {
  ...
  const saveInteraction = useCallback(async (draft: EditableNote, reason?: "checkbox" | "checkbox-saved") => {
    ...
    try {
      ...
      if (reason === "checkbox") void notifyTaskSave(draft, "checkbox-saved");
    } catch (reason) {
...
function SortableNoteCard({ note, assets, actionsDisabled, completedChecklistFilterEnabled, completedChecklistFilterRevision, completedChecklistFilterSnapshot, disabled, dropIndicatorEdge, resolveAssetUrl, onEdit, onTaskChange, onTaskCheckboxChange, onCollapsedChecklistSectionsChange, taskChangesDisabled, taskError }: {
  note: EditableNote;
  assets: Record<string, Asset>;
  actionsDisabled: boolean;
  completedChecklistFilterEnabled?: boolean;
          ...
          <div className={`note-card__viewport-frame${scrollState.scrollable ? " is-scrollable" : ""}${!scrollState.atTop ? " can-scroll-up" : ""}${!scrollState.atBottom ? " can-scroll-down" : ""}`}>
            <div className="note-card__viewport" onScroll={updateScrollState} ref={viewportRef}>
              <div className="note-card__content">
                {note.bodyMarkdown.trim() ? <MarkdownView completedChecklistFilterEnabled={completedChecklistFilterEnabled} completedChecklistFilterRevision={completedChecklistFilterRevision} completedChecklistFilterSnapshot={completedChecklistFilterSnapshot} collapsedChecklistSections={note.collapsedChecklistSections} firstHeadingPortalTarget={firstHeadingPortalTarget} markdown={note.bodyMarkdown} onCollapsedChecklistSectionsChange={onCollapsedChecklistSectionsChange} onTaskChange={onTaskChange} onTaskCheckboxChange={onTaskCheckboxChange} taskChangesDisabled={taskChangesDisabled} /> : null}
              </div>
            </div>
          </div>
        </div>
      </div>
      {taskError ? <p className="field-error inline-save-error" role="alert">{taskError}</p> : null}
...
function EmptyNoteGroup({ groupRank, disabled, filesDisabled = false, showCreate = true, onCreate, onFiles }: { groupRank?: number; disabled: boolean; filesDisabled?: boolean; showCreate?: boolean; onCreate: () => void; onFiles: (files: File[]) => void }) {
  const { isOver, setNodeRef } = useDroppable({
    id: groupRank === undefined ? "note-group:unavailable-trailing" : `note-group:${groupRank}`,
...
interface DraftNotePortalHost {
  element: HTMLDivElement;
  mountPortal: () => void;
}

type DraftNotePortalHosts = { current: Map<string, DraftNotePortalHost> };

function DraftNoteHostOrderer({ groups, hosts, notesRoot }: {
  groups: EditableNoteGroup[];
  hosts: DraftNotePortalHosts;
  notesRoot: { current: HTMLElement | null };
}) {
  useLayoutEffect(() => {
    for (const group of groups) {
      const groupElement = [...(notesRoot.current?.querySelectorAll<HTMLElement>(".note-group") ?? [])]
        .find((candidate) => candidate.dataset.noteGroupRank === String(group.groupRank));
      const grid = groupElement?.querySelector<HTMLElement>(".note-editors-grid");
      if (!grid) continue;

      group.notes.forEach((note, index) => {
        const portalHost = hosts.current.get(note.clientId);
        if (!portalHost) return;
        const current = grid.children.item(index);
        if (current !== portalHost.element) grid.insertBefore(portalHost.element, current);
        portalHost.mountPortal();
      });
    }
  }, [groups, hosts, notesRoot]);
  return null;
}

function SortableDraftNoteEditor({ note, autoFocus = false, disabled, dropIndicatorEdge, hosts, assets, storageLocked, canAddBlob, resolveAssetUrl, extraActions, takeInitialFiles, onAutoFocusConsumed, onChange, onProcessingChange }: {
  note: EditableNote;
  autoFocus?: boolean;
  disabled: boolean;
  dropIndicatorEdge?: NoteDropEdge | null;
  ...
  extraActions: ReactNode;
  takeInitialFiles?: () => File[];
  onAutoFocusConsumed?: (clientId: string) => void;
  onChange: (note: EditableNote) => void;
  ...
  const groupRank = noteGroupRank(note);
  const consumeAutoFocus = useCallback(() => onAutoFocusConsumed?.(note.clientId), [note.clientId, onAutoFocusConsumed]);
  const mountPortal = useCallback(() => setPortalReady(true), []);
  const { attributes, isDragging, isOver, listeners, setActivatorNodeRef, setNodeRef } = useSortable({
    id: `note:${note.clientId}`,
    animateLayoutChanges: () => false,
    attributes: { roleDescription: "перетаскиваемая заметка" },
    data: { type: "note", clientId: note.clientId, groupRank },
    disabled,
  });

  useLayoutEffect(() => {
    host.className = `note-editor-sortable${isDragging ? " is-dragging" : ""}${!isDragging && isOver ? " is-drop-target" : ""}`;
    host.dataset.noteId = note.clientId;
    host.dataset.shelfColumnSpan = note.doubleWidth ? "2" : "1";
  }, [host, isDragging, isOver, note.clientId, note.doubleWidth]);

  useLayoutEffect(() => {
    const portalHost = { element: host, mountPortal };
    hosts.current.set(note.clientId, portalHost);
    setNodeRef(host);
    return () => {
      if (hosts.current.get(note.clientId) === portalHost) hosts.current.delete(note.clientId);
      setNodeRef(null);
      host.remove();
    };
  }, [host, hosts, mountPortal, note.clientId, setNodeRef]);
...
function InlineGamePage({ game, notes, assets, platformSuggestions = [], tagSuggestions = [], storageLocked = false, canAddBlob, resolveAssetUrl, onSave, onDelete, sidebarLayoutMode = "side", onToggleSidebarLayout, completedChecklistFilterEnabled = false, onToggleCompletedChecklistFilter, noteInteractionSource }: GamePageProps & { game: Game }) {
  ...
  const [completedChecklistFilterRevision, setCompletedChecklistFilterRevision] = useState(0);
  ...
  const completedChecklistFilterTimeout = useRef<number | null>(null);
  ...
  if (previousCompletedChecklistFilterEnabledRef.current !== completedChecklistFilterEnabled) {
    previousCompletedChecklistFilterEnabledRef.current = completedChecklistFilterEnabled;
    completedChecklistFilterGenerationRef.current += 1;
  }
  const completedChecklistFilterGeneration = completedChecklistFilterGenerationRef.current;
  completedChecklistFilterEnabledRef.current = completedChecklistFilterEnabled;
  ...
  useEffect(() => {
    if (completedChecklistFilterEnabled || completedChecklistFilterTimeout.current === null) return;
    window.clearTimeout(completedChecklistFilterTimeout.current);
    completedChecklistFilterTimeout.current = null;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (completedChecklistFilterTimeout.current !== null) window.clearTimeout(completedChecklistFilterTimeout.current);
    };
  }, []);
  useEffect(() => {
  ...
  const scheduleCompletedChecklistFilterRefresh = useCallback((generation: number) => {
    if (!completedChecklistFilterPageMounted.current || !completedChecklistFilterEnabledRef.current || completedChecklistFilterGenerationRef.current !== generation) return;
    if (completedChecklistFilterTimeout.current !== null) window.clearTimeout(completedChecklistFilterTimeout.current);
    completedChecklistFilterTimeout.current = window.setTimeout(() => {
      completedChecklistFilterTimeout.current = null;
      if (!completedChecklistFilterPageMounted.current || !completedChecklistFilterEnabledRef.current || completedChecklistFilterGenerationRef.current !== generation) return;
      setCompletedChecklistFilterRevision((current) => current + 1);
    }, 60_000);
  }, []);
  const saveTaskNote = async (draft: EditableNote, reason?: "checkbox" | "checkbox-saved") => {
    if (reason === "checkbox-saved") {
      scheduleCompletedChecklistFilterRefresh(completedChecklistFilterGeneration);
      return;
    }
    ...
    try {
      ...
      } else if (reason === "checkbox") scheduleCompletedChecklistFilterRefresh(completedChecklistFilterGeneration);
    } finally {
  ...
  const groupInsertAvailable = noteGroups.map((group) => canPrepareNoteGroupAfter(visibleNotes, group.groupRank));
  const finalGroupRank = noteGroups.at(-1)?.groupRank;
  const trailingGroupAvailable = finalGroupRank === undefined || canAppendNoteGroupRank(finalGroupRank);
  const emptyGroupRank = trailingGroupAvailable ? nextEmptyNoteGroupRank(visibleNotes) : undefined;
...
function NewGamePage({ assets, platformSuggestions = [], tagSuggestions = [], storageLocked = false, canAddBlob, resolveAssetUrl, onCancel, onSave }: GamePageProps) {
  ...
  const notesEditorRef = useRef<HTMLElement>(null);
  const draftNotePortalHosts = useRef(new Map<string, DraftNotePortalHost>());
  ...
  const draftGroupInsertAvailable = draftNoteGroups.map((group) => canPrepareNoteGroupAfter(draftNotes, group.groupRank));
  const draftNoteLocations = new Map<string, { group: EditableNoteGroup; index: number }>();
  draftNoteGroups.forEach((group) => group.notes.forEach((note, index) => draftNoteLocations.set(note.clientId, { group, index })));
  const finalDraftGroupRank = draftNoteGroups.at(-1)?.groupRank;
  const trailingDraftGroupAvailable = finalDraftGroupRank === undefined || canAppendNoteGroupRank(finalDraftGroupRank);
  const emptyDraftGroupRank = trailingDraftGroupAvailable ? nextEmptyNoteGroupRank(draftNotes) : undefined;
  ...
  const addDraftNote = (groupRank: number | undefined, files: File[] = []) => {
...
export function GamePage(props: GamePageProps) {
  ...
  const completion = props.game
    ? { games: suggestions, excludeGameId: props.game.id }
    : { games: suggestions };
  return <GameLinkSuggestionsContext.Provider value={completion}>{props.mode === "game" && props.game
