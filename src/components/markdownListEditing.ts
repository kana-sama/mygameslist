...
export interface MarkdownListIndentEdit {
  value: string;
...
function findSelectionLineIndex(lines: readonly MarkdownLine[], position: number): number {
  for (let index = 0; index < lines.length; index += 1) {
    const lineEnd = lines[index].start + lines[index].content.length + lines[index].eol.length;
    if (position < lineEnd || index === lines.length - 1) return index;
  }
  return -1;
}

function findPreviousSiblingListLine(
  ...
  for (let index = lineIndex - 1; index >= 0; index -= 1) {
    ...
    if (!line.content.trim()) continue;
    const leadingWhitespace = /^[ \t]*/.exec(line.content)?.[0] ?? "";
    if (indentationWidth(leadingWhitespace) <= indentWidth) return null;
  }
  return null;
}

function listItemSubtreeEnd(lines: readonly MarkdownLine[], lineIndex: number, indentWidth: number): number {
  ...
  for (let index = lineIndex + 1; index < lines.length; index += 1) {
    ...
    const leadingWhitespace = /^[ \t]*/.exec(line.content)?.[0] ?? "";
    if (indentationWidth(leadingWhitespace) <= indentWidth) break;
    endIndex = index;
  }
  return endIndex;
}

function removeIndentationWidth(indent: string, width: number): string {
  if (width <= 0) return indent;

  const targetWidth = Math.max(0, indentationWidth(indent) - width);
  let prefix = "";
  let prefixWidth = 0;
  for (const character of indent) {
    const nextWidth = character === "\t"
      ? prefixWidth + 4 - prefixWidth % 4
      : prefixWidth + 1;
    if (nextWidth > targetWidth) break;
    prefix += character;
    prefixWidth = nextWidth;
    if (prefixWidth === targetWidth) return prefix;
  }
  return `${prefix}${" ".repeat(targetWidth - prefixWidth)}`;
}

interface IndentationPatch {
  start: number;
  removedLength: number;
  insertedLength: number;
}

function remapPosition(position: number, patches: readonly IndentationPatch[]): number {
  let delta = 0;
  for (const patch of patches) {
    const patchEnd = patch.start + patch.removedLength;
    if (position < patch.start) break;
    if (position <= patchEnd) return patch.start + delta + patch.insertedLength;
    delta += patch.insertedLength - patch.removedLength;
  }
  return position + delta;
}

export function resolveMarkdownListIndent(
  value: string,
  selectionStart: number,
  selectionEnd: number,
  direction: MarkdownListIndentDirection,
): MarkdownListIndentEdit | null {
  if (
    ...
    || direction !== "indent" && direction !== "outdent"
  ) return null;
  if (isInsideFencedMarkdownCode(value, selectionStart)) return null;

  const lines = splitMarkdownLines(value);
  const lineIndex = findSelectionLineIndex(lines, selectionStart);
  if (lineIndex < 0) return null;
  const parsed = parseListLine(lines[lineIndex].content);
  if (!parsed) return null;

  let indentationDelta: number;
  if (direction === "indent") {
    const previousSibling = findPreviousSiblingListLine(lines, lineIndex, parsed.indentWidth);
    if (!previousSibling) return null;
    indentationDelta = previousSibling.contentIndentWidth - parsed.indentWidth;
  } else {
    if (parsed.indentWidth === 0) return null;
    const parent = findParentListLine(lines, lineIndex, parsed.indentWidth);
    indentationDelta = parsed.indentWidth - (parent?.indentWidth ?? 0);
  }
  if (indentationDelta <= 0) return null;

  const selectionCollapsed = selectionStart === selectionEnd;
  const selectionEndProbe = selectionCollapsed ? selectionEnd : Math.max(selectionStart, selectionEnd - 1);
  let endLineIndex = findSelectionLineIndex(lines, selectionEndProbe);
  if (endLineIndex < lineIndex) return null;
  if (selectionCollapsed) {
    endLineIndex = listItemSubtreeEnd(lines, lineIndex, parsed.indentWidth);
  } else {
    for (let index = endLineIndex; index >= lineIndex; index -= 1) {
      const lastSelectedItem = parseListLine(lines[index].content);
      if (!lastSelectedItem) continue;
      const subtreeEnd = listItemSubtreeEnd(lines, index, lastSelectedItem.indentWidth);
      if (subtreeEnd >= endLineIndex) endLineIndex = subtreeEnd;
      break;
    }
  }

  for (let index = lineIndex; index <= endLineIndex; index += 1) {
    if (isInsideFencedMarkdownCode(value, lines[index].start)) return null;
  }

  for (let index = lineIndex; index <= endLineIndex; index += 1) {
    const line = lines[index];
    if (!line.content.trim()) continue;
    const indent = /^[ \t]*/.exec(line.content)?.[0] ?? "";
    if (indentationWidth(indent) < parsed.indentWidth) return null;
  }

  const patches: IndentationPatch[] = [];
  for (let index = lineIndex; index <= endLineIndex; index += 1) {
    const line = lines[index];
    if (!line.content.trim()) continue;
    const indent = /^[ \t]*/.exec(line.content)?.[0] ?? "";
    const nextIndent = direction === "indent"
      ? `${indent}${" ".repeat(indentationDelta)}`
      : removeIndentationWidth(indent, indentationDelta);
    patches.push({
      start: line.start,
      removedLength: indent.length,
      insertedLength: nextIndent.length,
    });
    line.content = `${nextIndent}${line.content.slice(indent.length)}`;
  }

  return {
    value: serializeLines(lines),
    selectionStart: remapPosition(selectionStart, patches),
    selectionEnd: remapPosition(selectionEnd, patches),
  };
}
